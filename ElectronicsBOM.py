...
# --- XHTML2PDF PER PDF DA HTML ---
try:
    from xhtml2pdf import pisa
except ImportError:
    print("ERRORE: Devi installare xhtml2pdf! Esegui: pip install xhtml2pdf")
    sys.exit(1)
        ...
        if CurrencyManager._rate is None or (now - CurrencyManager._last_update) > 86400:
            try:
                ...
                if CurrencyManager._rate is None: 
                    ...
                    QMessageBox.warning(None, "Error retreaving €/$ change rate", f"Error: {e}")
...
CATEGORIES = ["All", "Resistor", "Capacitor", "Inductor", "IC", "Microcontroller", 
              "Connector", "Transistor", "Diode", "Sensor", "Module", "Other"] #FIXME: add Resettable fuse

# --- FUNZIONE HELPER PER INIZIALIZZARE IL DB ---
def init_db(db_path):
    """
    Crea la connessione al database.
    NOTA: create_all() controlla se le tabelle esistono.
    Se esistono (database valido), NON fa nulla (non sovrascrive).
    Se non esistono (nuovo database), le crea.
    """
    if not os.path.exists(db_path):
        # Se stiamo aprendo un nuovo file, assicuriamoci che sia un file valido
        pass 
...
class NotesDialog(QDialog):
    def __init__(self, parent=None, project=None):
        ...
        self.setWindowTitle(f"Note Progetto: {project.name}")
        self.resize(500, 400)
        ...
        btn = QPushButton("Salva e Chiudi"); btn.clicked.connect(self.save_and_close)
        lay.addWidget(btn)
...
class ComponentDialog(QDialog):
    def __init__(self, parent=None, component=None):
        ...
        self.layout.addRow("Categoria:", self.inp_cat)
        self.layout.addRow("Mouser PN:", self.inp_m_pn)
        self.layout.addRow("JLCPCB Code:", self.inp_j_pn)
        self.layout.addRow("Descrizione:", self.inp_desc)
        self.layout.addRow("Quantità:", self.inp_qty)
        self.layout.addRow("Backup:", self.inp_backup)
        
        ll = QHBoxLayout()
        b_m = QPushButton(qta.icon('fa5s.external-link-alt'), "Apri Mouser"); b_m.clicked.connect(lambda: self.open_l(f"https://www.mouser.it/c/?q={self.inp_m_pn.text()}"))
        b_j = QPushButton(qta.icon('fa5s.external-link-alt'), "Apri JLCPCB"); b_j.clicked.connect(lambda: self.open_l(f"https://jlcpcb.com/partdetail/{self.inp_j_pn.text()}"))
        ll.addWidget(b_m); ll.addWidget(b_j)
        self.layout.addRow(QLabel("Link:"), ll)

        bb = QHBoxLayout()
        bs = QPushButton("Salva"); bs.clicked.connect(self.accept)
        bc = QPushButton("Annulla"); bc.clicked.connect(self.reject)
...
class SettingsDialog(QDialog):
    def __init__(self, parent=None):
        ...
        # DB Path Display
        current_db = self.settings.value("db_path", "Non impostato")
        ...
        form.addRow("Database Attuale:", self.lbl_db)
        
        layout.addLayout(form)
        
        # Change DB Logic
        # Qui usiamo un layout per mettere due bottoni: Apri Esistente (Safe) e Nuovo
        db_btn_layout = QHBoxLayout()
        btn_open_db = QPushButton("Seleziona Altro DB Esistente")
        btn_open_db.clicked.connect(self.select_existing_db)
        btn_open_db.setToolTip("Usa questo se il file DB esiste già (es. su Dropbox/Drive)")
        
        btn_new_db = QPushButton("Crea Nuovo DB")
        ...
        # Save Button
        b_save = QPushButton("Salva e Chiudi")
        b_save.clicked.connect(self.save_settings)
        layout.addWidget(b_save)

    def select_existing_db(self):
        # Usa getOpenFileName: Impedisce sovrascrittura, permette solo di scegliere file esistenti
        file_path, _ = QFileDialog.getOpenFileName(self, "Seleziona Database Esistente", "", "SQLite Database (*.db)")
        if file_path:
            self.settings.setValue("db_path", file_path)
            self.lbl_db.setText(file_path)
            QMessageBox.warning(self, "Riavvio Necessario", "Percorso aggiornato.\nRiavvia l'applicazione per caricare il nuovo database.")

    def create_new_db(self):
        # Usa getSaveFileName: Per creare nuovi file
        file_path, _ = QFileDialog.getSaveFileName(self, "Crea Nuovo Database", "my_bom_manager_new.db", "SQLite Database (*.db)")
        if file_path:
            self.settings.setValue("db_path", file_path)
            self.lbl_db.setText(file_path)
            QMessageBox.warning(self, "Riavvio Necessario", "Nuovo percorso impostato.\nRiavvia l'applicazione per creare e usare il nuovo database.")
...
class MainWindow(QMainWindow):
    def __init__(self, session_factory):
        ...
        self.session = session_factory() # Crea la sessione dal factory passato
        self.threadpool = QThreadPool()
        self.current_project = None
        self.tm = 0.0
        self.tj = 0.0
        self.hybrid_total = 0.0
        self.init_ui()
        self.load_projects()
    ...
    def init_ui(self):
        ...
        pb = QHBoxLayout()
        ba = QPushButton("Nuovo"); ba.clicked.connect(self.add_project)
        bd = QPushButton("Elimina"); bd.clicked.connect(self.delete_project)
        ...
        self.lbl_t = QLabel("Seleziona Progetto"); self.lbl_t.setStyleSheet("font-size:18px; font-weight:bold; border:none; background:none;")
        hl.addWidget(self.lbl_t)
        
        btn_edit = QPushButton(qta.icon('fa5s.pen'), "")
        btn_edit.setToolTip("Modifica nome progetto"); btn_edit.setFixedSize(30, 30); btn_edit.clicked.connect(self.rename_project)
        hl.addWidget(btn_edit)

        btn_notes = QPushButton(qta.icon('fa5s.sticky-note', color="#FFC107"), "")
        btn_notes.setToolTip("Note Progetto"); btn_notes.setFixedSize(30, 30); btn_notes.clicked.connect(self.open_notes)
        hl.addWidget(btn_notes)
        hl.addStretch()

        # Export Menu
        btn_exp = QPushButton(qta.icon('fa5s.file-export'), "Esporta")
        menu_exp = QMenu()
        act_csv = QAction("Esporta CSV (Dettagliato)", self); act_csv.triggered.connect(self.export_csv)
        act_pdf = QAction("Esporta PDF (Orizzontale)", self); act_pdf.triggered.connect(self.export_pdf)
        ...
        hl.addWidget(QLabel("Filtra:")); hl.addWidget(self.cmb_f); hl.addSpacing(10)
        ...
        self.cols = ["ID", "Mouser PN", "JLC Code", "Cat", "Desc", "Qtà", "Mouser", "JLCPCB"]
        ...
        # --- UI TWEAKS PER EVITARE "..." ---
        self.tab.setWordWrap(True) # Testo a capo
        self.tab.setTextElideMode(Qt.TextElideMode.ElideNone) # Disabilita i puntini "..."
        
        header = self.tab.horizontalHeader()
        header.setSectionResizeMode(QHeaderView.ResizeMode.ResizeToContents) # Default: stretto il giusto
        header.setSectionResizeMode(4, QHeaderView.ResizeMode.Stretch) # Descrizione: prende tutto lo spazio

        rl.addWidget(self.tab)

        act = QHBoxLayout()
        bac = QPushButton("Aggiungi Componente"); bac.clicked.connect(self.add_component)
        brc = QPushButton("Rimuovi"); brc.clicked.connect(self.del_component)
        act.addWidget(bac); act.addWidget(brc); act.addStretch()
        rl.addLayout(act)

        self.lbl_stat = QLabel("Totali: -"); self.lbl_stat.setStyleSheet("background:#007acc; padding:10px; font-weight:bold;")
    ...
    def get_last_refresh_date(self):
        dates = []
        if not self.current_project: return "Mai"
        for c in self.current_project.components:
            if c.last_update: dates.append(c.last_update)
        return max(dates) if dates else "Mai"

    def calculate_unit(self, total, qty):
        if qty <= 0: return 0.0
        return total / qty

    # --- LOGICA PROGETTI & NOTE ---
    def rename_project(self):
        if not self.current_project: return
        new_n, ok = QInputDialog.getText(self, "Rinomina", "Nuovo nome:", text=self.current_project.name)
        if ok and new_n:
            try:
                self.current_project.name = new_n; self.session.commit()
                self.lbl_t.setText(new_n); self.load_projects()
            except Exception as e: QMessageBox.warning(self, "Errore", f"Errore (nome duplicato?): {e}")
    ...
    def export_csv(self):
        if not self.current_project: return
        path, _ = QFileDialog.getSaveFileName(self, "Salva CSV", f"{self.current_project.name}.csv", "CSV Files (*.csv)")
        if path:
            try:
                ...
                QMessageBox.information(self, "Export", "CSV Salvato!")
            except Exception as e: QMessageBox.critical(self, "Errore", str(e))

# --- EXPORT PDF (COMPACT CARD LAYOUT - "SLIM" VERSION) ---
    def export_pdf(self):
        if not self.current_project: return
        path, _ = QFileDialog.getSaveFileName(self, "Salva PDF", f"{self.current_project.name}.pdf", "PDF Files (*.pdf)")
        if not path: return
        ...
        for c in self.current_project.components:
            ...
            # COSTRUZIONE DELLA SCHEDA (COMPATTA)
            cards_html += f"""
            <div class="card">
        ...
        if self.current_project.notes:
            notes_html = f"<div class='notes'><h3>Note:</h3><p>{self.current_project.notes.replace(chr(10), '<br>')}</p></div>"
        ...
        <head>
            <style>
                ...
                .card {{
                    ...
                    margin-bottom: 8px; /* Meno spazio tra le schede */
                ...
                .cat-badge {{ 
                    font-size: 8px; background: #666; color: white; 
                    padding: 1px 4px; border-radius: 3px; font-weight: normal; margin-right: 5px;
                }}
        ...
        <body>
            ...
            <div class="meta">
                Generato: {datetime.now().strftime('%d/%m/%Y %H:%M')} | Dati aggiornati: {self.get_last_refresh_date()}
            </div>
        ...
        try:
            ...
            if pisa_status.err:
                QMessageBox.critical(self, "Errore", "Errore creazione PDF")
            else:
                QMessageBox.information(self, "Export", "PDF creato con successo!")
        except Exception as e:
    ...
    def add_project(self):
        n, ok = QInputDialog.getText(self, "Nuovo", "Nome:"); 
    ...
    def delete_project(self):
        ...
        if QMessageBox.question(self,"Delete","Sicuro?", QMessageBox.StandardButton.Yes|QMessageBox.StandardButton.No)==QMessageBox.StandardButton.Yes:
            ...
            self.load_projects(); self.tab.setRowCount(0); self.lbl_t.setText("Seleziona Progetto")
    ...
    def refresh_prices(self):
        try:
            ...
            QMessageBox.critical(self, "Error!", str(e))    # MINE
    ...
    def update_db_and_ui(self, data):
        ...
        if c:
            ...
            c.last_update = data['timestamp'] # Salva data refresh
            self.session.commit()
    ...
    def calc_total(self):
        ...
        if self.current_project:
            for c in self.current_project.components:
                ...
                # Totale Ibrido (Logica Best Price)
...
def main():
    ...
    # --- SETUP DATABASE E PRIMA ESECUZIONE ---
    settings = QSettings("MySoft", "BOMManager")
    db_path = settings.value("db_path", "")
    
    # Se non c'è un percorso DB salvato
    if not db_path:
        # Crea un QMessageBox personalizzato per la scelta iniziale
        msg = QMessageBox()
        msg.setWindowTitle("BOM Manager - Configurazione Iniziale")
        msg.setText("Benvenuto! Non hai un database configurato.")
        msg.setInformativeText("Vuoi creare un nuovo file o aprirne uno esistente (es. dal Cloud)?")
        
        # Aggiungi pulsanti personalizzati
        btn_new = msg.addButton("✨ Crea Nuovo", QMessageBox.ButtonRole.ActionRole)
        btn_open = msg.addButton("📂 Apri Esistente", QMessageBox.ButtonRole.ActionRole)
        btn_cancel = msg.addButton("Esci", QMessageBox.ButtonRole.RejectRole)
        ...
        if msg.clickedButton() == btn_cancel:
            ...
            # SAVE FILE NAME (Crea Nuovo)
            file_path, _ = QFileDialog.getSaveFileName(None, "Crea Nuovo Database", "my_bom_manager.db", "SQLite Database (*.db)")
            if file_path:
                settings.setValue("db_path", file_path)
                db_path = file_path
                with open(db_path, 'w'): pass # Crea file
            else:
                sys.exit(0)

        elif msg.clickedButton() == btn_open:
            # OPEN FILE NAME (Apri Esistente - Sicuro, niente sovrascrittura)
            file_path, _ = QFileDialog.getOpenFileName(None, "Seleziona Database Esistente", "", "SQLite Database (*.db)")
            if file_path:
                settings.setValue("db_path", file_path)
                db_path = file_path
            else:
    ...
    # Verifica esistenza file prima di caricare (se è stato cancellato manualmente)
    if not os.path.exists(db_path):
        QMessageBox.critical(None, "Errore Database", f"Il file database non esiste:\n{db_path}\n\nRiavvia e seleziona 'Crea Nuovo' o 'Apri Esistente'.")
        settings.remove("db_path") # Resetta impostazione
        sys.exit(1)

    # Inizializza DB e Sessione
